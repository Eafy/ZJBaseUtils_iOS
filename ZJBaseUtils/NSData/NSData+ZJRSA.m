...
@implementation NSData (ZJRSA)

+ (NSData *)zj_stripPublicKeyHeader:(NSData *)d_key
{
...
+ (NSData *)zj_stripPrivateKeyHeader:(NSData *)d_key
{
    if (d_key == nil) return(nil);
...
+ (SecKeyRef)addPublicKey:(NSString *)key
{
    ...
    NSData *data = [key zj_base64Decode];
    data = [self zj_stripPublicKeyHeader:data];
    if(!data){
        return nil;
    }
...
+ (SecKeyRef)addPrivateKey:(NSString *)key
{
    ...
    NSData *data = [key zj_base64Decode];
    data = [self zj_stripPrivateKeyHeader:data];
    if(!data){
        return nil;
    }
...
- (NSData *)zj_rsaEncryptWithPublicKey:(NSString *)pubKey
{
    ...
    SecKeyRef keyRef = [NSData addPublicKey:pubKey];
    if(!keyRef){
        return nil;
    }
    return [self zj_rsaEncryptWithKeyRef:keyRef isSign:NO];
}
...
- (NSData *)zj_rsaEncryptWithPrivateKey:(NSString *)privKey
{
    ...
    SecKeyRef keyRef = [NSData addPrivateKey:privKey];
    if(!keyRef){
        return nil;
    }
    return [self zj_rsaEncryptWithKeyRef:keyRef isSign:YES];
}
...
- (NSData *)zj_rsaDecryptWithPublicKey:(NSString *)pubKey
{
    ...
    return [self zj_rsaDecrypttWithKeyRef:keyRef];
}
