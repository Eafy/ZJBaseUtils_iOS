...
NSString *const kZJSSKeychainClassKey = @"labl";
NSString *const kZJSSKeychainDescriptionKey = @"desc";
NSString *const kZJSSKeychainLabelKey = @"labl";
NSString *const kZJSSKeychainLastModifiedKey = @"mdat";
NSString *const kZJSSKeychainWhereKey = @"svce";

#if __IPHONE_4_0 && TARGET_OS_IPHONE  
CFTypeRef ZJSSKeychainAccessibilityType = NULL;
#endif

@interface ZJSAMKeychain ()
+ (NSMutableDictionary *)_queryForService:(NSString *)service account:(NSString *)account;
@end

@implementation ZJSAMKeychain
...
+ (NSArray *)accountsForService:(NSString *)service error:(NSError **)error {
    OSStatus status = ZJSSKeychainErrorBadArguments;
    ...
    if (status != noErr && error != NULL) {
		*error = [NSError errorWithDomain:kZJSSKeychainErrorDomain code:status userInfo:nil];
		return nil;
	}
...
#if __IPHONE_4_0 && TARGET_OS_IPHONE
		if (ZJSSKeychainAccessibilityType) {
#if __has_feature(objc_arc)
...
+ (CFTypeRef)accessibilityType {
	return ZJSSKeychainAccessibilityType;
}
...
+ (void)setAccessibilityType:(CFTypeRef)accessibilityType {
	...
	if (ZJSSKeychainAccessibilityType) {
		CFRelease(ZJSSKeychainAccessibilityType);
	}
	ZJSSKeychainAccessibilityType = accessibilityType;
}
#endif
