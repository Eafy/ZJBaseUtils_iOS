...
NSString *const kJMSSKeychainErrorDomain = @"org.jimi.sskeychain";

NSString *const kJMSSKeychainAccountKey = @"acct";
NSString *const kJMSSKeychainCreatedAtKey = @"cdat";
NSString *const kJMSSKeychainClassKey = @"labl";
NSString *const kJMSSKeychainDescriptionKey = @"desc";
NSString *const kJMSSKeychainLabelKey = @"labl";
NSString *const kJMSSKeychainLastModifiedKey = @"mdat";
NSString *const kJMSSKeychainWhereKey = @"svce";

#if __IPHONE_4_0 && TARGET_OS_IPHONE  
CFTypeRef JMSSKeychainAccessibilityType = NULL;
#endif

@interface JMSAMKeychain ()
+ (NSMutableDictionary *)_queryForService:(NSString *)service account:(NSString *)account;
@end

@implementation JMSAMKeychain
...
+ (NSArray *)accountsForService:(NSString *)service error:(NSError **)error {
    OSStatus status = JMSSKeychainErrorBadArguments;
    ...
    if (status != noErr && error != NULL) {
		*error = [NSError errorWithDomain:kJMSSKeychainErrorDomain code:status userInfo:nil];
		return nil;
	}
...
#if __IPHONE_4_0 && TARGET_OS_IPHONE
		if (JMSSKeychainAccessibilityType) {
#if __has_feature(objc_arc)
...
+ (CFTypeRef)accessibilityType {
	return JMSSKeychainAccessibilityType;
}
...
+ (void)setAccessibilityType:(CFTypeRef)accessibilityType {
	...
	if (JMSSKeychainAccessibilityType) {
		CFRelease(JMSSKeychainAccessibilityType);
	}
	JMSSKeychainAccessibilityType = accessibilityType;
}
#endif
