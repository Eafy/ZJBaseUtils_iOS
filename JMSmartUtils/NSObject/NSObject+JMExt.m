...
#import "NSObject+JMExt.h"
#import <objc/runtime.h>

@implementation NSObject (JMExt)

- (NSArray *)jm_propertyList
{
    return [[self class] jm_propertyList];
}

- (NSArray *)jm_ivarList
{
    return [[self class] jm_ivarList];
}
...
{
    return [[self class] jm_protocolMap];
}

- (void)jm_replaceMethod:(SEL)oldMethod newMethod:(SEL)newMethod
{
    [[self class] jm_replaceMethod:oldMethod newMethod:newMethod];
}


#pragma mark - StaticAPI

+ (NSDictionary *)jm_dictionaryWithProperty:(objc_property_t)property
{
...
+ (NSArray *)jm_propertyList
{
    ...
    for (int i=0; i<propertyCount; i++) {
        [propertieArray addObject:({
            NSDictionary *dictionary = [self jm_dictionaryWithProperty:properties[i]];
            dictionary;
        })];
    }
...
+ (NSArray *)jm_propertyFormatList
{
    ...
    NSArray *properties = [[self class] jm_propertyList];
...
/// 类型
/// @param cString 类型
+ (NSString *)jm_decodeType:(const char *)cString
{
    ...
    } else
    {
        if ([[result substringToIndex:1] isEqualToString:@"^"]) {
            result = [NSString stringWithFormat:@"%@ *",
                      [NSString jm_decodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
    return result;
}
...
+ (NSArray *)jm_ivarList
{
    ...
    for (int i = 0; i < outCount; i++) {
        NSString *type = [[self class] jm_decodeType:ivar_getTypeEncoding(ivars[i])];
