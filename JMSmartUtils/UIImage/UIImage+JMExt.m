...
- (NSString *)jm_saveToDocumentsWithName:(NSString *)picName dirName:(NSString *)dirName
{
    ...
    NSString *documentsPath = [NSFileManager jm_documentsPath];
    if (self) {
        ...
        if (dirName && dirName.length > 0) {
            ...
            if ([self jm_saveWithPath:filePath]) {
                return filePath;
            }
        } else {
...
{
    return [self jm_rotate:UIImageOrientationDownMirrored];
}

- (UIImage *)jm_rotateHorizontal
{
    return [self jm_rotate:UIImageOrientationUpMirrored];
}
...
{
    return [self jm_rotatedByRadians:(M_PI * (degrees) / 180.0)];
}
...
+ (UIImage *)jm_imageWithColor:(UIColor *)color
{
    return [self jm_imageWithColor:color size:CGSizeMake(1.0, 1.0)];
}
