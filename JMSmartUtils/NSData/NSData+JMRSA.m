...
@implementation NSData (JMRSA)

+ (NSData *)jm_stripPublicKeyHeader:(NSData *)d_key
{
...
+ (NSData *)jm_stripPrivateKeyHeader:(NSData *)d_key
{
    if (d_key == nil) return(nil);
...
+ (SecKeyRef)addPublicKey:(NSString *)key
{
    ...
    NSData *data = [key jm_base64Decode];
    data = [self jm_stripPublicKeyHeader:data];
    if(!data){
        return nil;
    }
...
+ (SecKeyRef)addPrivateKey:(NSString *)key
{
    ...
    NSData *data = [key jm_base64Decode];
    data = [self jm_stripPrivateKeyHeader:data];
    if(!data){
        return nil;
    }
...
- (NSData *)jm_rsaEncryptWithPublicKey:(NSString *)pubKey
{
    ...
    return [self jm_rsaEncryptWithKeyRef:keyRef isSign:NO];
}
...
- (NSData *)jm_rsaEncryptWithPrivateKey:(NSString *)privKey
{
    ...
    return [self jm_rsaEncryptWithKeyRef:keyRef isSign:YES];
}
...
- (NSData *)jm_rsaDecryptWithPublicKey:(NSString *)pubKey
{
    ...
    return [self jm_rsaDecrypttWithKeyRef:keyRef];
}
